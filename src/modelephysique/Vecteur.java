 ...
 * vecteur.
 * 
 * Cette classe est une version 2d modifiée de la classe SVector3d ecrite par
 * Simon Vezina.
 * 
 * @author Simon Vézina
 * @author Simon Tran
...
public class Vecteur {
	...
	/**
	 * Construit un vecteur depuis une longeur et un angle
	 * 
	 * @param longueur
	 ...
	 * @return le vecteur
	 */
	public static Vecteur vecteurAvecAngle(double longueur, double angle) {
		return new Vecteur(longueur * Math.cos(angle), longueur * Math.sin(angle));
	}
	 ...
	 */
	public static Vecteur vecteurNull() {
		return new Vecteur(0, 0);
	}
	
	/**
	 * Retourne l'angle du vecteur en radiants
	 * @return	l'angle en radiants
	 */
	...
	public Vecteur normalise() throws Exception {
		...
		if (mod < EPSILON)
			throw new Exception("Erreur Vecteur: Le vecteur " + this.toString()
					+ " étant nul ou presque nul ne peut pas etre normalisé.");
		else
	...
	/**
	 * Retourne une forme ligne à partir d'une position
	 * 
	 * @param position
	 *            La position d'où il faut dessiner la ligne
	 * @return la ligne
	 */
	public Line2D.Double ligne(Vecteur position) {
		return new Line2D.Double(position.getX(), position.getY(), position.getX() + getX(), position.getY() + getY());
	}
	
	/**
	 * Retourne une forme ligne partant de l'origine
	 * @return	la ligne
	 */
	public Line2D.Double ligne(){
		return ligne(Vecteur.vecteurNull());
	}
	
	
	public static Vecteur projectionOrthogonale(Vecteur vecteur1, Vecteur  vecteur2) {
		double terme1 = prodScalaire(vecteur1, vecteur2);
		double terme2 = prodScalaire(vecteur2, vecteur2);
		return vecteur2.multiplie((terme1 / terme2) );
	}
	
	
	public Vecteur rotate(double angle) {
		double cos = Math.cos(angle), sin = Math.sin(angle);
	//	Vecteur vecteur = new Vecteur(x * cos + y *-sin, x * -sin + y *cos);
		Vecteur vecteur = new Vecteur(x * cos - y * sin, y * cos + x * sin);
